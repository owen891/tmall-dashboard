...
def _error(error, status=422):
    return failure(getattr(error, 'code', 'VALIDATION_ERROR'), str(error), status=status)
...
def run_scan_job(job_id):
    try:
        ...
        return success(ImportScanService.run_job_once(job_id, force=bool(payload.get('force'))))

...
![经营总览](docs/assets/readme/overview.png)
...
浏览器门禁覆盖七个 API 驱动主页面、响应式视口、图表像素输出、错误状态和横向溢出；发布审计会检查脏工作树、演示/真实批次混合、数据库完整性和来源血缘。

## 已知限制

> **生产数据阻断：** 当前发布前审计确认生产库仍有 **5,674 条**历史或演示日事实缺少可信、可验证的 observation/lineage。正式用于生产决策前，必须核实、隔离或补录真实来源；禁止推测性回填。数量以 `py -3 scripts/release_audit.py --database data/dashboard.db --strict` 的最新输出为准。

...
最新验证：446 项完整回归通过，包含 37 个子测试；全部正式 JS 语法检查、七页静态门禁和浏览器 PRD 门禁通过。跨店回归覆盖日度导入、KPI、趋势、总览、商品、对比、推广查询及导入批次审计/撤销；周/月事实、目标、生命周期、动作及依赖单店旧表的工具接口仍是单店结构，非 `default` 店铺请求或周/月导入会返回 `UNSUPPORTED_SCOPE`，避免静默串店。

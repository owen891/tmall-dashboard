...
class ImportScanConflictError(RuntimeError):
    def __init__(self, message, code='SCAN_CONFLICT'):
...
def _is_stale_importing(row, now=None):
    ...
    return updated_at <= reference - timedelta(seconds=LEASE_SECONDS)
...
def _next_cron_run(cron_expr, after):
    ...
    candidate = after.astimezone(CRON_TIMEZONE).replace(second=0, microsecond=0) + timedelta(minutes=1)
...
class ImportScanService:
    ...
    def _discover(cls, job, force=False):
        ...
        for entry in os.scandir(folder):
            ...
            if stat.st_size <= 0 or stat.st_size > max_size:
                ...
                continue
            if not force and now - stat.st_mtime < 60:
                cls._record_unstable(job['id'], canonical, entry.name, stat.st_size, stat.st_mtime_ns)
                continue
            with get_db() as conn:
    ...
    def _upsert_discovered(cls, job_id, item):
        ...
        with get_db() as conn:
            ...
            if existing:
                ...
                if _is_stale_importing(existing):
                    conn.execute(
                        '''UPDATE import_scan_files SET status='discovered', error_code=NULL,
                           error_message=NULL, updated_at=? WHERE id=?''',
                        (now, existing['id']),
                    )
                    conn.commit()
                    return {**dict(existing), 'status': 'discovered', 'updated_at': now}, True
    ...
    def _acquire_lease(cls, job_id):
        ...
        if result.rowcount != 1:
            ...
            raise ImportScanConflictError('扫描任务正在运行，请稍后重试', 'SCAN_RUNNING')
        return token

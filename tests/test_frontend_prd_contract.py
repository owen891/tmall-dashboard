...
class FrontendPrdContractTests(unittest.TestCase):
    ...
    def test_goals_default_to_monthly_decision_view_and_use_period_picker(self):
        page = self.read('frontend/ui_demo/pages/goals.html')
        adapter = self.read('frontend/ui_demo/assets/goals-live.js')
        self.assertIn('data-goals-level-filter', page)
        self.assertIn('value="month" selected', page)
        self.assertIn('data-goals-period-picker', page)
        self.assertIn('configurePeriodPicker', adapter)
        self.assertIn('let loadToken = 0', adapter)
        self.assertIn('if (token !== loadToken) return', adapter)
        self.assertIn('data-goals-settings-summary', page)
        self.assertIn('suggested_annual_target', page)
        self.assertIn('readonly', page)
        self.assertIn('增长倍率（本次生成）', page)
        self.assertNotIn('name="growth_multiplier" type="number" min="0.01" step="0.01" value="1.1" readonly', page)
        self.assertIn('/api/settings', adapter)
        self.assertIn('prepareMonthAdjustment', adapter)
        self.assertIn('textContent = \'调整\'', adapter)
        self.assertIn('const renderDataState =', adapter)
        self.assertIn("renderDataState('loading')", adapter)
        self.assertIn("error.status === 404 ? 'no-data' : 'calculation-failed'", adapter)
        self.assertIn('retry: () => load(Number(form.elements.year.value))', adapter)

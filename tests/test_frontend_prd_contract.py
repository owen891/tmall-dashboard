...
class FrontendPrdContractTests(unittest.TestCase):
    ...
    def test_schedule_surfaces_use_domain_mutation_endpoints(self):
        shell = self.read('frontend/ui_demo/assets/shell.js')
        manage = self.read('frontend/ui_demo/assets/manage-live.js')
        self.assertIn('requestDomainApi', shell)
        self.assertIn("/api/manage/schedules", shell)
        self.assertIn("DemoApi.domainRequest('/api/manage/schedules')", manage)
        self.assertIn('/api/manage/schedules/${Number(item.id)}', manage)
        self.assertNotIn("'/api/scheduled_tasks'", shell)
    ...
    def test_ui_audit_regressions_are_covered(self):
        ...
        self.assertIn('name="schedule_name"', shell)

(function () {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (saving) return;
    if (Object.keys(settingsPayload.capabilities || {}).length > 0 && !DemoApi.can(settingsPayload, 'can_edit')) { status.textContent = '当前设置不可修改'; return; }
    const data = new FormData(form);
    saving = true;
    if (saveButton) { saveButton.disabled = true; saveButton.setAttribute('aria-busy', 'true'); saveButton.textContent = '保存中…'; }
    ...
    try {
      ...
      renderState('calculation-failed', { message: error.message || '保存失败' });
    } finally {
      saving = false;
      if (saveButton) { saveButton.disabled = false; saveButton.removeAttribute('aria-busy'); saveButton.textContent = '保存设置'; }
      if (discardButton) discardButton.disabled = !isDirty;
    }
  });
  ...
  const formatScanFormError = (error) => {
    ...
    if (message.includes('cron_expr')) return '扫描计划格式不正确。请使用 5 段 cron 表达式，例如：0 8 * * *。';
    return message;
  };
  ...
  const formatScanTime = (value) => value ? new Intl.DateTimeFormat('zh-CN', { dateStyle: 'medium', timeStyle: 'medium', hour12: false, timeZone: 'Asia/Shanghai' }).format(new Date(value)) : '--';
  ...
  function runScanJob(job) {
    ...
    mutateScan(`正在扫描 ${job.task_name}…`, () => DemoApi.domainRequest(`/api/import-scans/${Number(job.id)}/run`, { method: 'POST' })).then((response) => {
      ...
      showToast(summary, { duration: result.failed_count || result.blocked_count ? 6000 : 4500 });
    });
  }
  ...
  async function loadScanDetails(job) {
    ...
    try {
      ...
      const body = document.querySelector('[data-scan-files]'); body.replaceChildren();
      if (!(files.data || []).length) { const row = document.createElement('tr'); const cell = appendTextCell(row, '暂无文件记录'); cell.colSpan = 5; body.appendChild(row); return; }
      (files.data || []).slice(0, 20).forEach((item) => {
        const row = document.createElement('tr');
  ...
  scanForm?.addEventListener('submit', (event) => {
    ...
    setScanFormStatus('');
    mutateScan(jobId ? '正在更新任务…' : '正在创建任务…', async () => { const response = await DemoApi.domainRequest(jobId ? `/api/import-scans/${Number(jobId)}` : '/api/import-scans', { method: jobId ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); scanDialog.close(); return response; }, (error) => { setScanStatus(''); setScanFormStatus(formatScanFormError(error)); }).then((response) => { if (response) showToast(jobId ? `扫描任务已更新：${payload.task_name}` : `扫描任务已创建：${payload.task_name}`); });
  });
  load();
  loadScanJobs();
})();

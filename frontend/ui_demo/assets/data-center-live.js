(function () {
  ...
  confirmButton?.addEventListener('click', async () => {
    ...
    try {
      ...
      const summary = `已导入 ${results.length}/${pending.length} 个文件；新增 ${inserted}，更新 ${updated}；DMP补齐 ${resolution.fallback_filled}，参考 ${resolution.reference_only}，冲突 ${resolution.conflicts}`;
  ...
  async function loadHistory() {
    ...
    try {
      ...
      history.replaceChildren(...batches.map((batch) => {
        ...
        const qualityText = `${DemoLabels.label('quality', batch.quality_summary?.conclusion, batch.status === 'completed' ? '通过' : '--')} · 补齐 ${resolution.fallback_filled || 0} · 参考 ${resolution.reference_only || 0} · 冲突 ${resolution.conflicts || 0} · 汇总剔除 ${batch.quality_summary?.excluded_summary_rows || 0}`;
        ...
        button.addEventListener('click', async () => { const impact = `影响：撤销 ${batch.source_filename}，恢复该批次覆盖前的数据；${batch.inserted_count || 0} 条新增记录将被移除，${batch.updated_count || 0} 条更新将回退。`; if (!window.confirm(`确认撤销该批次？\n${impact}`)) return; await DemoApi.domainRequest(`/api/imports/${encodeURIComponent(batch.id)}/revert`, {method:'POST'}); text(status, '批次已撤销。'); loadHistory(); }); cell.appendChild(button); row.appendChild(cell); return row;
      }));

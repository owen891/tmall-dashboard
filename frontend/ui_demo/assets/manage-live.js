(function () {
  ...
  function renderSchedules() {
    ...
    if (!state.schedules.length) return tableMessage('[data-manage-scheduled]', 5, '暂无定时任务');
    state.schedules.forEach((item) => {
      const row = document.createElement('tr'); const title = document.createElement('div'); title.className = 'table-name'; const name = document.createElement('strong'); name.textContent = item.task_name || '未命名任务'; const pattern = document.createElement('span'); pattern.textContent = item.file_pattern || '*.xlsx'; title.append(name, pattern);
      ...
      appendCell(row, actionGroup([
        button('', 'pencil', () => openSchedule(item), '编辑调度'),
        button('', Number(item.enabled) === 1 ? 'pause' : 'play', () => toggleSchedule(item), Number(item.enabled) === 1 ? '停用调度' : '启用调度'),
        button('', 'play-circle', () => runSchedule(item), '立即执行'),
        button('', 'trash-2', () => removeSchedule(item), '删除调度'),
      ])); body.appendChild(row);
    });
  }
  function renderLogs() {
  ...
  async function load() {
    ...
    try {
      ...
      if (token !== state.token) return;
      tableMessage('[data-manage-tasks]', 6, '任务加载失败'); tableMessage('[data-manage-kpis]', 6, 'KPI 加载失败'); tableMessage('[data-manage-scheduled]', 5, '调度任务加载失败'); tableMessage('[data-manage-logs]', 4, '操作日志加载失败');
  ...
  function openSchedule(item) {
    const dialog = $('[data-manage-schedule-dialog]'); const form = $('[data-manage-schedule-form]'); state.scheduleId = item?.id ?? null; form.reset(); form.elements.file_pattern.value = '*.xlsx';
    $('#manage-schedule-title').textContent = state.scheduleId ? '编辑调度' : '新增调度';

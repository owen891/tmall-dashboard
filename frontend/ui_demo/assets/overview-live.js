(function () {
  ...
  const matrixColumnsStorageKey = 'tmall-overview-matrix-columns';
  ...
  function matrixValue(row, column) {
    ...
    const value = column.key === 'source_batch_id' ? source.source_filename || source.source_batch_id || source.data_source : row[column.key];
    if (value == null || value === '') return '不可计算';
  ...
  function renderKpis(overview, comparison) {
    const metrics = [
    ...
    metrics.forEach(([key, formatter]) => {
      ...
      const value = key === 'visitors' ? (overview.visitors ?? overview.product_visitors) : key === 'ad_roi' && overview.ad_roi == null
  ...
  function renderHomeProducts(payload) {
    ...
    root.replaceChildren(...products.map((product, index) => {
      ...
      const localThumbnail = localThumbnailUrl(product.product_id);
      const image = localThumbnail ? document.createElement('img') : document.createElement('span'); image.className = 'overview-v2-product-image';
      if (localThumbnail) { image.src = localThumbnail; image.alt = product.title || product.product_id || ''; image.loading = 'eager'; image.decoding = 'async'; }
  ...
  async function load(detail) {
    ...
    renderKpis(overviewResponse.data, comparison); renderContext(overviewResponse.data); renderTrend(rows); renderHomeProducts(products); renderTargets(targets); await loadGoalLayers(state); renderAnomalies(anomalies); renderHomeMatrix(matrix.data || {});
  ...
  window.addEventListener('tmall:export', (event) => {
    ...
    if (Object.keys(overviewPayload?.capabilities || {}).length && !DemoApi.can(overviewPayload, 'can_export')) {

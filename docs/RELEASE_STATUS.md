...
当前代码可以在本机或受信内网运行，但当前版本仍不可直接上线。代码回归和运行恢复门禁通过；正式发布前必须完成工作区分类、真实库/演示库分离、来源血缘回填和生产库备份。
...
- `data/dashboard.db` 同时存在 3 个演示批次和 4 个真实批次，演示库与真实库尚未物理分离。
...
- 演示数据只允许写入 `data/demo/dashboard.db`，使用 `py scripts/seed_demo_data.py --demo-database`。

...
- 演示数据不是前端 mock：8 个 `DEMO-*` 商品驱动正式 API、筛选、聚合、生命周期、动作和复盘；演示商品批次记录 4712 条变更并可真实撤销。目标演示遵守保护规则：空库生成调整与锁定示例，已有真实目标的当前库不写入演示调整。
- 演示种子幂等，只更新明确演示命名空间；非演示商品事实、真实目标和真实周期复盘有保护性回归测试。

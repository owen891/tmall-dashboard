...
Do not seed demonstrations into `data/dashboard.db`. Use the isolated target:

```powershell
py -3 scripts/seed_demo_data.py --demo-database
```

Tests and controlled recovery exercises may pass `--database <path>`. Writing
the repository production path requires the explicit
`--allow-production-database` override and should not be part of normal release
operations.

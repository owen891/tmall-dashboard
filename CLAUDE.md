# 嗨贝海数据仪表盘 - AI 执行规范
# 集成 SCALE OS v10.0 认知脚手架

## 🧠 认知脚手架

> 从客观存在着的实际事物出发，让事实规定判断，让现实修正理论。
> 没有调查就没有发言权。

### 反惰性机制

禁止以下惰性行为，一旦识别，立即回到认知工作流：
- 暴力重试（同一个方法重复 3 次以上）
- 甩锅用户（说"你只需要..."而不解决问题）
- 工具闲置（放着 grep/cat/ls 不用而猜测代码）
- 忙碌假象（显示很多 activity 但没实质进展）
- 被动等待（不主动检查状态）
- 越权擅改（没确认就修改核心配置）
...
```
1. 📡 探索研究 — 先摸清上下文，禁止上来就写代码
2. 📋 规划决策 — 基于探索制定方案，集中兵力一次一事
3. 🔨 执行实施 — 契约确认后编码，TDD 闭环驱动
4. ✅ 验证测试 — 工具验证 > 脑补，5步完成门控
5. 📈 沉淀优化 — 修一个点查一个面，知识沉淀闭环
```

...
function showPeriodDetail(period, mouseEvent) {
    ...
    // 获取该周期的 KPI 数据
    fetch(`/api/kpi?dim=${STATE.dim}&period=${period}`)
        .then(r => r.json())
        .then(data => {
...
async function loadKPI(dim, period, prevPeriod) {
    ...
    const kpiMap = {
        gmv:          { label: '总GMV',     fmt: v => fmtWan(v),     key: 'gmv' },
...
function renderProductTable() {
    ...
    if (thead) {
        ...
        // 构建分组行和列头行
        let groupRowHtml = '<th class="th-rank" rowspan="2"><input type="checkbox" id="selectAll" onchange="toggleSelectAll(this.checked)" title="全选"></th><th class="th-product" rowspan="2">商品信息</th>';
        let headerRowHtml = '';
        let currentGroup = '';
        let groupSpan = 0;
...
async function loadHealthDashboard(period) {
    ...
    const [statsData, alertData] = await Promise.all([
        apiFetch(`/api/health?period=${period}`),
        apiFetch(`/api/health?period=${period}&level=预警`),
    ]);
    ...
    if (!alertData || !alertData.products || alertData.products.length === 0) {
        wrapperEl.innerHTML = '<div style="text-align:center;color:#64748B;padding:40px;">暂无预警商品</div>';
        return;
    }

    const sorted = [...alertData.products].sort((a, b) => (a.health_score || 0) - (b.health_score || 0));
...
function showHealthDetail(product_id) {
    ...
    apiFetch(`/api/health?period=${period}`).then(data => {
        ...
        content.innerHTML = `
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">
                ${product.image_url ? `<img src="${product.image_url}" style="width:48px;height:48px;border-radius:8px;object-fit:cover;background:#1E293B;">` : ''}
                <div style="flex:1;">
                    <div style="font-size:1rem;font-weight:600;color:#F1F5F9;">${product.title || '--'}</div>
                    <div style="font-size:0.8rem;color:#64748B;">${product.tier || ''} &middot; ${product.style || ''}</div>
                </div>
...
async function loadTargetProgress(period) {
    ...
    predictValueEl.textContent = `预计月底GSV ${fmtWan(predictedGSV)}`;
...
async function loadAlerts(period) {
    if (STATE.dim !== 'monthly') return;
    const data = await apiFetch(`/api/alerts?period=${period}`);
...
async function executeCurrentTool() {
    ...
    try {
        ...
        if (!resp.ok) {
            resultArea.innerHTML = '<div class="tool-result-msg error">❌ 请求失败: HTTP ' + resp.status + '</div>';
            return;
        }
